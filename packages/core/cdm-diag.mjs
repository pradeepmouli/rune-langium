...
const files = readdirSync(join(cwd, cdmDir)).filter(f => f.endsWith('.rosetta'));
...
// Also show some sample error messages in full 
console.log('\n--- Sample full error messages ---');
for (const doc of docs.slice(0,5)) {
  const errs = (doc.diagnostics ?? []).filter(d => d.severity === 1).slice(0,3);
  if (errs.length) {
    console.log(doc.uri.path.split('/').pop() + ':');
