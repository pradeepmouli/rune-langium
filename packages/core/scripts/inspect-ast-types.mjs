...
for (const iface of astTypes.interfaces) {
  for (const prop of iface.properties ?? []) {
    const t = prop.type;
    const typeName = typeof t === 'string' ? t : t && typeof t === 'object' && 'name' in t ? String(t.name) : null;
...
console.log('Unresolved property types (datatype rules etc.):', Object.fromEntries(unresolvedPropTypes));
