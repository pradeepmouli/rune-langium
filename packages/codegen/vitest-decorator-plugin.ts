...
// carve-out got.
//
// Exported (not inlined in packages/codegen/vitest.config.ts) so the
// repo-root vitest.config.ts — used by `pnpm run test:coverage`, which
// still resolves packages/codegen's test files (see its own `test.include`
// glob) — can register the exact same transform. Without this, running
// codegen's tests through the root config hit this file's own
// SyntaxError on any decorator-using source file, since the root config
// has no other way to know about codegen's decorator syntax.
