...
export const ThenCheckSchema = z
  ...
  .refine(
    (data) => ((__pipe) => (runeCount((() => { const __seen = new Set(); return (__pipe ?? []).filter((value) => { const key = runeValueKey(value); if (__seen.has(key)) return false; __seen.add(key); return true; }); })()) > 0))(data.items),
