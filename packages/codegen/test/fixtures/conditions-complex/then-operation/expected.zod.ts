...
export const ThenCheckSchema = z
  ...
  .refine(
    (data) => ((__pipe) => (runeCompare(runeCount((() => { const __seen = new Set(); return runeList(__pipe).filter((value) => { const key = runeValueKey(value); if (__seen.has(key)) return false; __seen.add(key); return true; }); })()), 0, (a, b) => a > b, "all")))(data.items),
