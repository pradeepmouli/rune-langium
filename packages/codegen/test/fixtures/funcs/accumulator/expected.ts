...
// Source namespace: test.funcs.accumulator
...
export function CollectItems(input: { value: number }): number[] {
  input = { ...input, value: ((value) => { if (value == null) throw new Error("Argument 'value' requires a value"); return value; })(runeSingle(input.value)) };
  let result: number[] = [];
  {
    const __assignmentValue = runeList(input.value);
    if (Array.isArray(__assignmentValue)) result.push(...__assignmentValue);
    else if (__assignmentValue != null) result.push(__assignmentValue);
  }
  return result;
}
