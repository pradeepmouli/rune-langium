...
- Function expression helpers live in `packages/codegen/src/expr/`; preserve cardinality and metadata across call and assignment boundaries. Validate changed semantics with parsed Rune, strict compilation of generated modules, and runtime assertions. Nested set/add targets retain numeric cardinality bounds and share runtime checks with function outputs; appends check the combined collection before mutating it. Nested Temporal fields use strings at function boundaries through the shared `RuneFuncData` structural mapping. See the codegen README.
