...
1. **`$type` is retained as a literal discriminant** on every domain read interface (`$type: 'Data'`) + in `toDomain` output, so `AnyDomain` is a DISCRIMINATED union (consumers `switch (d.$type)`; the repository indexes by kind). This is the user-directed correction (2026-06-06): id-related metadata is keyed on `$type`. (The **0.6.0** emitter — on langium-zod `develop`, NOT the published 0.5.4 — filters `$type` at `domain.ts:135`; un-filter it. See the version note in the current-state map.)
...
4d. **Expressions are projected FULLY — inline-union read projection added to the emitter (decided 2026-06-08, SUPERSEDES the earlier "opaque" option).** The emitter had a documented gap (`domain.ts:80-83`): inline property-level unions of rich objects "pass through unchanged" as the raw AST sub-object (`$`-internals leak, type/runtime mismatch) — violating 4b. Rune's only such field is `typeRef: RosettaSuperCall | RosettaSymbolReference` (`ast.ts:2532`), inside the expression grammar, reached by `expression: RosettaExpression` on 5 nodes (Condition/Operation/…). Investigation 2026-06-08: the editor edits expressions as TEXT (`operations[0].expression.$cstText` + a synthetic `expressionText`), never the parsed tree; and langium-zod generates an interface for EVERY descriptor, so merely stripping the `expression` field would still emit a broken `typeRef` interface. **User decision: project expressions fully** (not opaque). The fix is an emitter capability (a NEW 3D-1 task): `domainReadExpr`'s `union` arm projects each member at runtime via the master `toDomain($type)` dispatch (object-members → `toDomain`, primitives → pass-through), matching the union TYPE `domainTsType` already emits. This closes the `:80-83` gap and lets the full expression tree project losslessly. `domain-surfaces.json` then includes the expression nodes (full surface, no exclude).
...
- **Linkage:** `langium-zod ^0.5.4` in BOTH `core` + `visual-editor` devDeps; no override yet. Sibling repo at `/Users/pmouli/GitHub.nosync/active/ts/langium-zod` (monorepo `langium-zod-workspace`, package `packages/langium-zod`). Link path from rune workspace root is **`../langium-zod/packages/langium-zod`** (ONE `..` — `ts/rune-langium` → `ts/langium-zod`; the earlier `../../` was wrong).

---

## Phase 3D-1 — langium-zod emitter changes (foundational; workspace-linked into rune)

> **Audit re-verified 2026-06-08 against the live `develop` (0.6.0) emitter.** Already present (do NOT rebuild): `toDomainX`/`XDomain` interfaces, `AnyDomain` + master `toDomain` switch, union projections, type-qualified write accessors (`addX`/`removeXAt`/`setX`), rename/merge overlays. 3D-1 is exactly FOUR deltas — retain `$type` (`:135`), brand cross-refs (`:53` read-type / `:95` write-type → `Ref<'T'>`), add `emitRepository` (absent), add `applyNormalizations` (absent).

### Task 0: Pre-flight — branch, build, link, scope-check

**Repo:** langium-zod (`develop` → new branch) + rune (`pnpm-workspace.yaml`).

- [ ] **Step 1: Branch** `feat/domain-discriminant-repository` off langium-zod **`develop`** (0.6.0 tip — `git -C ../langium-zod checkout develop && git -C ../langium-zod checkout -b feat/domain-discriminant-repository`).
- [ ] **Step 2: Confirm the scope decision (constraint 4d)** — `rg -n ':\s*\w+\s*\|\s*\w+' packages/core/src/generated/ast.ts | rg -v '\$container'` to enumerate inline-union props. Confirm the only type-surface hit is `typeRef` (expression grammar); record that `domain-surfaces.json` will keep expressions opaque (`$cstText`), so no inline-union-of-rich-objects field enters the surface. (If a NEW non-expression inline-union surfaces, escalate before Task 1 — it would reopen the project-vs-opaque decision.)
- [ ] **Step 3: Sanity-link early (optional but recommended)** — add the `link:../langium-zod/packages/langium-zod` override now, `pnpm --filter langium-zod build && pnpm install`, confirm `node_modules/langium-zod` resolves to the branch build. This de-risks the Task-3 link step and lets Tasks 1/2/2b be smoke-tested against rune's real grammar as they land.
...
- [ ] **Step 1: Link the local langium-zod** — add a `pnpm-workspace.yaml` override pointing `langium-zod` at the local `feat/domain-discriminant-repository` build (`link:../langium-zod/packages/langium-zod` — ONE `..`; verify it resolves from the workspace root). `pnpm --filter langium-zod build` first so the linked `dist` carries the branch changes. `pnpm install`; confirm `node_modules/langium-zod` resolves to the branch build (with `$type` retention + `emitRepository`).
...
- [ ] **Step 1: Release langium-zod** — changeset (minor: `$type` retention + branded refs + `emitRepository` + normalizations), merge `feat/domain-discriminant-repository` → `develop`, publish the **next version after 0.6.0 (`0.6.1` or `0.7.0` — 0.6.0 is already taken by PR #68/#69)**. CI green (the #68 `@types/node` fix holds).
