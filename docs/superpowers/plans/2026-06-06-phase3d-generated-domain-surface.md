...
4d. **Expression bodies are OPAQUE on the domain surface (decided 2026-06-08).** The emitter has a documented gap (`domain.ts:80-83`): inline property-level unions of rich objects "pass through unchanged" — i.e. emitted as the raw AST sub-object with `$`-internals leaking, which would violate 4b. Rune hits exactly one such field on the type surface, `typeRef: RosettaSuperCall | RosettaSymbolReference` (`ast.ts:2532`), in the expression grammar. Decision: the domain surface treats expression bodies as **opaque** (`expression.$cstText` string, as the editor already edits them) rather than projecting the parsed expression tree — so no inline-union-of-rich-objects field enters the surface and the `:80-83` gap never triggers. This is a `domain-surfaces.json` scoping choice, NOT emitter surgery; a Task-3 assertion verifies the surface contains no inline-union-of-rich-objects field. (Projecting inline unions in the emitter — the deferred "later task" — is explicitly OUT of 3D scope.)
