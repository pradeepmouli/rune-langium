...
The closure's real `Data`/`Choice`/`RosettaEnumeration`/`RosettaTypeAlias` AST nodes — regardless of which namespace they actually belong to — get indexed into one synthetic `NamespaceWalkResult`-shaped object (`dataByName`/`choiceByName`/`enumByName`/`typeAliasByName`, keyed by bare name), then handed to the existing, completely unmodified `emitNamespace()` (zod target). From that function's own perspective every dependency is now "local," so it never emits a cross-namespace `import` in the first place — no changes to `zod-emitter.ts`'s cross-namespace import logic are needed.
