...
A structural renderer covering the full `RosettaExpression` union (48 members, verified against `packages/core/src/generated/ast.ts`): the 8-level precedence chain (`ThenOperationRule` → `OrOperation` → `AndOperation` → `EqualityOperationRule` → `AdditiveOperation` → `MultiplicativeOperation` → `BinaryOperationRule` → `UnaryOperation`/`PrimaryExpression`) and all node types under `UnaryOperation`/`PrimaryExpression` (navigation, aggregation, higher-order functions, control flow, constructors, literals).

**Scope correction (verified during planning):** the 4 mapping-qualifier types (`RosettaMapTestExistsExpression`, `RosettaMapTestAbsentExpression`, `RosettaMapTestEqualityOperation`, `RosettaAttributeReference`) are **NOT** members of the `RosettaExpression` union — they appear only inside synonym mapping bodies, which render-core already preserves via the non-value-body CST fallback (restored in `12b69772`). They are unreachable from the 3 expression call sites and out of scope.

**Safety net retained (spec amendment):** render-core's call sites wrap `renderExpression` in a try/catch — on an unknown `$type` (e.g. a future grammar addition) `renderExpression` throws, and the call site falls back to the existing `$cstText`/`$cstNode.text` passthrough. This applies the Task-7 regression lesson from the synonym-guard removal (PR #357 Copilot finding #2): a fallback that routes unknown constructs to CST must degrade, never corrupt. `exprText()` therefore survives as the fallback arm, no longer the primary path.
...
**Extension point (spec amendment — simplified from the earlier callback design):** `renderExpression` natively supports one generic escape-hatch leaf, `{ $type: 'RawDsl', text: string }`, rendered verbatim. It is not UI-specific — it's a "pre-rendered DSL fragment" leaf. The visual-editor adapter converts `Placeholder` (→ `RawDsl` with the `___` marker) and `Unsupported` (→ `RawDsl` with `rawText`) to it. Render-core's own 3 callers never produce it.

**Custom parser note (verified):** `services.parser.LangiumParser` in this project is `RuneDslParser`, a subclass whose `parse(input, options)` applies `insertImplicitBrackets(input)` before delegating (and passes `options` through, so `{ rule: ... }` works). This is *correct* for bare expressions — implicit-bracket insertion targets expression-level functional operators (`extract`/`filter`/…), so bare snippets get the same treatment they'd get inside a document.
...
- **Modify** `packages/codegen/src/emit/rosetta/rosetta-render-core.ts` — the 3 `exprText()` call sites route through `renderExpression()` first, with `exprText()` retained as the try/catch CST fallback arm (see safety-net amendment above).
