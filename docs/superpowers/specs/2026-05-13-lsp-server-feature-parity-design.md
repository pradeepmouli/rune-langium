...
- [ ] **T2.4** Fix transport lifecycle: clear `this.transport` in `webSocketClose` so reconnect to a non-evicted instance works. Change `apps/lsp-worker/src/index.ts`'s `handleWsUpgrade` (`:190`) to key the DO **per connection** (decided 2026-08-08, §7) instead of by `workspaceId` alone — mint a per-connection id (e.g. `${workspaceId}:${connectionId}`) so each tab gets its own DO instance and Langium server.
...
- [ ] **T2.9 (new, 2026-08-08, studio-side)** In `apps/studio/src/providers/LspProvider.tsx` (`syncWorkspaceFiles`, `:106`), scope what gets pushed into the DO's Langium index to **currently-open editor documents only** (decided §7) — not the whole loaded workspace/corpus. Close a tab → the DO should eventually drop that document from its index too (via `didClose`, subject to T2.4's per-connection DO now owning a much smaller working set). This is the mitigation for the 128 MB isolate-ceiling risk; without it, T2.1–T2.8 wire a correct server that can still be crashed by a large workspace.
...
- [x] **T4.1 (decided 2026-08-08): no feature flag.** Ship directly, consistent with this repo's `feedback_no_migration_before_live` guidance — the studio isn't live-with-users yet, so gating ceremony for a state with no real prior users is unwarranted overhead. T4.1's original flag-implementation work is dropped.
...
| **(new, 2026-08-08) 128 MB per-isolate memory ceiling vs. `syncWorkspaceFiles` pushing the whole loaded corpus into the DO's Langium index.** `LspProvider.tsx:106` pushes every loaded workspace file as a `didOpen` today. Isolate memory is shared across *all* DO instances co-resident in that isolate, not per-DO. This repo has already hit almost this exact shape once: `project_curated_503_dep_graph_no_link` records a ~238 MB closure blowing CF's un-raisable 128 MB cap in `/api/parse`. | **Decided (2026-08-08): cap sync to currently-open editor documents only** (T2.9), not the whole loaded workspace. Combined with per-connection DO keying (below), this bounds the working set per DO to what one tab has open. Still needs a load test at realistic open-tab-count/file-size before rollout. |
| **(new, 2026-08-08) Multi-tab fan-out.** `apps/lsp-worker/src/index.ts:190` keys the DO by `workspaceId` alone, so N browser tabs on one workspace share one DO instance and one `LSPServer`. `@lspeasy/server`'s own docs say never to share one `LSPServer` instance across multiple transports/connections; request ids can also collide across clients sharing a socket-bound transport. | **Decided (2026-08-08): key the DO per connection**, not per `workspaceId` (T2.4). Accepts N× memory/Langium construction per workspace across tabs — mitigated by the open-editors-only sync cap above, which keeps each per-connection DO's own working set small. |
| **(new, 2026-08-08) Cold-start latency on hibernation wake is unmeasured.** A full `import()` + `createRuneLspServer()` + `startLanguageServer` + N-document-build replay runs on the user's next keystroke after any wake. | **Decided (2026-08-08): build the replay path and keep hibernation for V1** (§8b) — replay is needed regardless for redeploy/crash eviction, so ship it first and measure real wake latency in production. Revisit `server.accept()` only if measured latency proves unacceptable. |
...
**Decided (2026-08-08): not adopted for V1.** Keep `state.acceptWebSocket(server)` (hibernation) and build the cold-wake replay path (§3, T2.3) — it's needed regardless of this choice, since DOs are still evicted on redeploy, crash, and (per Cloudflare's docs) after 70–140s of idle time even in non-hibernatable mode. Measure real wake latency in production after V1 ships; only then, if it proves unacceptable, revisit this section.

Kept here as the documented fallback: Cloudflare's Hibernation API is what makes §7's cold-start-latency risk real — a resident, non-hibernating DO (`server.accept()` instead) keeps the Langium workspace warm for the WebSocket's lifetime, with no replay cost in the steady state. Per Cloudflare's own pricing example (100 always-on WS DOs ≈ $410/mo), a resident DO bills duration for the connection's full lifetime — but at the studio's current scale (~1 concurrent session, several hours/day) this is roughly $1–2/mo, not a meaningful cost. If adopted later, it's a small, low-risk change: flipping `acceptWebSocket` → `accept()` demotes replay to a rare cold path instead of a routine one, rather than removing the need for it.
...
- [ ] **(new)** Multi-tab scenario tested: two tabs on the same workspace get independent, working DO instances (per-connection keying, T2.4)
- [ ] **(new)** Open-editors-only sync scoping tested: closing a tab drops that document from the DO's Langium index (T2.9); a large loaded-but-unopened workspace does not inflate the DO's memory
...
Net effect on this spec: §1, §3, §4, §5, and Phase 2/3/4 of §6 were substantively rewritten; §7 gained four new risk rows; §8 gained a tuning-knob subsection (§8b); §9's checklist gained five new items. Phase 1 (§6) and the recommendation in §8 did not change — both were already correct.

**Follow-up (2026-08-08, same day): the four open decisions this revision originally left unresolved were walked through and settled** — sync scope (§7, T2.9: cap to open editors only, not the whole loaded workspace), multi-tab DO keying (§3, §7, T2.4: per-connection, not per-`workspaceId`), hibernation vs. resident DO (§8b: keep hibernation, build replay first, measure before reconsidering `server.accept()`), and rollout gating (§6 T4.1: no feature flag, ship directly). T2.9 is a new studio-side task (`apps/studio/src/providers/LspProvider.tsx`) that wasn't in the original DO-only task list. This spec is no longer blocked on open decisions; the next step is an implementation plan.
