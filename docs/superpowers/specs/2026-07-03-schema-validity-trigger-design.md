...
- **Oracle gaps (schema passes, render UNPARSEABLE):** `RosettaSynonymBody` with no non-empty alternative among {values, hints, merge, mappingLogic, metaValues} (the render-synonym-body.ts:252 throw case); `RosettaMappingInstance` with none of {default, set, when}. Each is ALSO a z2f blind spot — forms report them valid today. ~~Empty `Choice`/`Enum`~~ — RETRACTED (Stream-2 finding): rune's grammar gives all three body lists bare `*` cardinality, so empty bodies PARSE; they are semantically incomplete (language-validation territory), not parse-invalid, and the schema must keep accepting them per the hard invariant.
...
1. **Comma-list min-1 — AMENDED (Stream-2 finding, 2026-07-03): verification-only.** The min-occurrence analysis (incl. the comma-list idiom) ALREADY SHIPPED in langium-zod 0.10.0/0.10.1 (`feat/array-min-occurrence` + PR #96 fragment fix). Moreover the original expectation here was WRONG: rune's grammar uses bare `*` cardinality for `Choice.attributes`, `RosettaEnumeration.enumValues`, AND `Data.attributes` — empty bodies are grammatically parseable, so `min(1)` on any of them would violate the hard invariant. It conflated parseability with semantic completeness: `min(1)` belongs only where the GRAMMAR demands ≥1; "an empty choice is incomplete" is the language-validation layer's job (LSP diagnostics), not the parse-shape schema's. Stream 2 verifies existing min-1 output against rune's real grammar and empirically confirms which empty bodies parse (and what validation diagnostics they carry).
