...
7. **Given** the studio loaded against the CDM corpus, **When** `performance.memory.usedJSHeapSize` is sampled after the workspace has finished parsing, **Then** the post-migration value is **measurably lower** than the pre-migration baseline. The plan captures the baseline once and adds a Playwright assertion that the live measurement does not exceed `baseline + 5%` headroom. Methodology: launch Chromium with `--enable-precise-memory-info`, sample `performance.memory.usedJSHeapSize` after the `data-testid="workspace-loaded"` selector appears.
...
| The runtime memory-footprint win (separate from bundle size) may not materialize if the curated corpus is still loaded eagerly by some other code path. | Phase 2 ships a Playwright test that asserts `performance.memory.usedJSHeapSize` against a captured baseline (US2 AS-7). Test fails if the post-migration heap is not measurably smaller. `--enable-precise-memory-info` Chromium flag required; `measureUserAgentSpecificMemory()` is the cross-origin-isolated alternative if precision proves insufficient. |
