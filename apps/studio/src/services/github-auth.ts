...
export const initDeviceFlow = withInstrumentation(
  async function initDeviceFlow(authBase: string): Promise<InitResult> {
    let res: Response;
    try {
    ...
    // Output carries deviceCode/userCode (short-lived auth secrets) — never captured.
  },
  { op: 'initDeviceFlow' }
);

export const pollDeviceFlow = withInstrumentation(
  async function pollDeviceFlow(authBase: string, deviceCode: string): Promise<PollResult> {
    let res: Response;
    try {
    ...
    // `deviceCode` (input) and `accessToken` (output on success) are literal credentials — never captured.
  },
  { op: 'pollDeviceFlow' }
);
...
export const storeWorkspaceToken = withInstrumentation(
  async function storeWorkspaceToken(fs: OpfsFs, workspaceId: string, token: string): Promise<void> {
    await fs.writeFile(`/${workspaceId}/${TOKEN_PATH}`, token);
    // `token` is a literal GitHub PAT — never captured.
  },
  { op: 'storeWorkspaceToken' }
);

export const loadWorkspaceToken = withInstrumentation(
  async function loadWorkspaceToken(fs: OpfsFs, workspaceId: string): Promise<string | null> {
    try {
    ...
    // Output is the token itself — never captured.
  },
  { op: 'loadWorkspaceToken' }
);

export const deleteWorkspaceToken = withInstrumentation(
  async function deleteWorkspaceToken(fs: OpfsFs, workspaceId: string): Promise<void> {
    try {
