...
2. **Functions ↔ operations mapping (both directions)**: Rune `func` ↔ OpenAPI path operation. Inbound: `paths.*.{method}` → `RosettaFunction` (`operationId` → sanitized func name; parameters + requestBody schema → inputs with required→cardinality; 2xx response schema → output; summary/description → definition; parameter constraints → conditions). Outbound: the inverse. **Plus CRUD conventions (user, 2026-07-04)**: inbound, recognize standard REST CRUD shapes (`GET /xs`, `GET /xs/{id}`, `POST /xs`, `PUT|PATCH /xs/{id}`, `DELETE /xs/{id}`) and map them to conventionally-named funcs over the resource type (`Create_X`/`Read_X`/`Update_X`/`Delete_X`/`List_X` — matching CDM's existing `Create_*` func naming idiom) rather than opaque operationId-derived names; outbound, an opt-in flag to generate the standard CRUD operation set for selected types. DESIGN-TIME VERIFICATION: whether the grammar permits synonym annotations on funcs — if not, the operation↔func correspondence needs a different carrier (definition text or naming convention); settle in the Phase 2b brainstorm.
