...
## Phase 3 (adopted 2026-07-06, user-directed): XSD reader + emitter

**Order**: XSD before GraphQL (user, 2026-07-06) — direct domain relevance (FpML is natively XSD) over general utility. Both are two-way (import AND export), matching the OpenAPI precedent, for the same reason: a single-artifact round-trip oracle (Rune → XSD → Rune) is the strongest test available, and the outbound side gets a real facet/constraint recognizer symmetric to the inbound translator.

**TypeScript was explicitly deprioritized** (user, 2026-07-06): bare TS types carry no self-contained constraint semantics without a Zod companion, and anyone with Zod schemas can already emit JSON Schema and use the existing importer — a dedicated reader would be mostly tree-sitter-syntax risk for marginal value.

**Parser: NOT tree-sitter.** XSD is a document format (XML), not a language with real grammar/statements — same category as JSON Schema/OpenAPI, not SQL. `fast-xml-parser@5.9.3` (user-selected 2026-07-06; MIT, pure JS, actively maintained — 344M downloads/month, latest release weeks old at time of writing) is the INBOUND parser. No mature typed-XSD-vocabulary package exists (checked the real npm registry: the closest candidate, `xsd-parser`, is a single-maintainer, 2-version, ~35-downloads/month hobby project; `xsd2jsonschema`, which would let the reader delegate to the existing JSON Schema reader via conversion, is unmaintained since 2020) — same conclusion as the SQL reader's own `node-types.json` situation: hand-roll a small, typed walker over XSD's own well-known, fixed vocabulary (`xs:complexType`, `xs:simpleType`, `xs:restriction`, `xs:sequence`, `xs:choice`, `xs:all`, `xs:extension`, `xs:enumeration`, `xs:attribute`, plus the numeric/string restriction facets), the same way `json-schema-reader.ts` hand-rolls JSON Schema vocabulary walking. **AMENDED (as-built correction):** the outbound `-t xsd` emitter (`src/emit/xsd-emitter.ts`) does NOT use `fast-xml-parser`'s companion `XMLBuilder` for serialization — it hand-assembles deterministic XML strings directly (no `XMLBuilder` import at all), the same way `json-schema-emitter.ts`'s own `serializeJson` hand-rolls its formatter rather than pulling in a generic serializer. This gives full control over exact formatting (including the namespace-declaration shape fixed by a later PR-review finding) and deterministic, golden-file-style output for the round-trip oracle tests — `fast-xml-parser` is therefore an INBOUND-only dependency in the shipped implementation, not "one dependency for both directions" as originally planned.

**Two real design decisions grounded via a direct spike against `fast-xml-parser@5.9.3`** (not assumed — verified with a representative multi-construct sample XSD covering complexType/simpleType/restriction/enumeration/sequence/choice/extension/attribute):

1. **`isArray` is mandatory, not optional.** By default, a tag occurring once parses to a plain object; the SAME tag occurring more than once parses to an array — the exact single-vs-many ambiguity bug class that hit the SQL reader twice (BETWEEN low/high positional guessing, invocation-parameter fallback search). Configure the parser's `isArray` callback to force `xs:element`, `xs:complexType`, `xs:simpleType`, `xs:enumeration`, and `xs:attribute` to ALWAYS be arrays regardless of occurrence count — eliminated at the parser-configuration boundary, proactively, rather than discovered through a bug later.
2. **Namespace prefixes are literal and arbitrary** — a real document may use `xsd:`, no prefix (default namespace), or any other alias for `http://www.w3.org/2001/XMLSchema`. The reader resolves tag/type-reference lookups via the DECLARED namespace URI (read from the root element's `xmlns:*` attributes into a prefix→URI map), never hardcoded `'xs:'`-prefixed string literals.

**No local `.xsd` fixtures exist** (checked: `.resources/rune-fpml/` already contains FpML translated into `.rosetta` syntax, not raw XSD) — hand-written representative fixtures are the test basis (same as every prior reader's own fixture-based tests), with a skip-guarded corpus test added later if a real `.xsd` file becomes locally available, matching the established `.resources`-absence convention.

**Vocabulary mapping (MVP scope):**
- Top-level named `xs:complexType` → Rune `type` (Data)
- `xs:simpleType` whose `xs:restriction` has ONLY `xs:enumeration` children (no numeric/pattern facets) → Rune `enum`
- `xs:element` / `xs:attribute` (child of `xs:sequence`/`xs:choice`/`xs:all`, or a complexType's own attribute) → attribute; built-in XSD type → Rune builtin (`xs:string`→`string`, `xs:decimal`/`xs:double`/`xs:float`→`number`, `xs:int`/`xs:integer`/`xs:long`/`xs:short`→`int`, `xs:boolean`→`boolean`, `xs:date`→`date`, `xs:dateTime`→`dateTime`); a reference to another named `complexType`/enum-shaped `simpleType` → a typed attribute referencing that Rune type
- `minOccurs`/`maxOccurs` → cardinality (`minOccurs="0"` → optional; default/`1` → mandatory; `maxOccurs="unbounded"` or `&gt;1` → `(0..*)`/`(1..*)`)
- `xs:choice` → the established oneOf/choice idiom (Phase 1 precedent): each choice member becomes a `(0..1)` attribute, plus a `required choice a, b, ...` condition
- `xs:extension` (via `xs:complexContent`) → `extends`, base type reference resolved via the namespace map
- `xs:restriction`'s numeric/string facets (`xs:minInclusive`/`xs:maxInclusive`/`xs:minExclusive`/`xs:maxExclusive` → `range`; `xs:pattern` → `pattern` stub per the established no-expression-level-regex rule; `xs:minLength`/`xs:maxLength`/`xs:length` → `length`) → `ConstraintIR`, the same vocabulary every other reader already translates through
- Unsupported/out-of-MVP-scope constructs — stub + diagnostic, never silently dropped: `xs:union`, `xs:import`/`xs:include` (multi-file schema composition; single-document import only for MVP), substitution groups, abstract types/elements, `xs:group`/`xs:attributeGroup` references, mixed content

**Outbound emitter** (`-t xsd`, new `TARGET_DESCRIPTORS` entry + `src/emit/xsd-emitter.ts`): Data → `xs:complexType`; Enum → `xs:simpleType` + `xs:restriction base="xs:string"` + `xs:enumeration`; Choice/oneOf conditions → `xs:choice`; `extends` → `xs:complexContent`/`xs:extension`; a constraint RECOGNIZER (condition AST → `ConstraintIR` → XSD restriction facets, the inverse of the inbound translator, mirroring the OpenAPI emitter's own recognizer) for comparison/range/length conditions specifically — CONSERVATIVE per the established per-bound-exclusivity lesson (reject to an unrepresentable/opaque fallback rather than guess on anything ambiguous).

**Oracle**: single-artifact Rune → XSD → Rune identity (structure + recognized conditions), the OpenAPI-precedent pattern — achievable here because, unlike the outbound JSON Schema emitter, both directions are being built together and XSD's restriction facets have a real, literal representation to round-trip through.
