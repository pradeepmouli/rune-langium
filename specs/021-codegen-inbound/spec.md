...
**Parser: NOT tree-sitter.** XSD is a document format (XML), not a language with real grammar/statements — same category as JSON Schema/OpenAPI, not SQL. `fast-xml-parser@5.9.3` (user-selected 2026-07-06; MIT, pure JS, actively maintained — 344M downloads/month, latest release weeks old at time of writing) is the parser AND the outbound serializer (its companion `XMLBuilder`, same package — one dependency for both directions). No mature typed-XSD-vocabulary package exists (checked the real npm registry: the closest candidate, `xsd-parser`, is a single-maintainer, 2-version, ~35-downloads/month hobby project; `xsd2jsonschema`, which would let the reader delegate to the existing JSON Schema reader via conversion, is unmaintained since 2020) — same conclusion as the SQL reader's own `node-types.json` situation: hand-roll a small, typed walker over XSD's own well-known, fixed vocabulary (`xs:complexType`, `xs:simpleType`, `xs:restriction`, `xs:sequence`, `xs:choice`, `xs:all`, `xs:extension`, `xs:enumeration`, `xs:attribute`, plus the numeric/string restriction facets), the same way `json-schema-reader.ts` hand-rolls JSON Schema vocabulary walking.
