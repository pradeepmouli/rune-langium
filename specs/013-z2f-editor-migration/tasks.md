...
- [ ] T076 DRY-cleanup (FR-008 + R1): delete `packages/visual-editor/src/schemas/form-schemas.ts`. Update every editor under `src/components/editors/` that previously imported from it to use the matching AST schema (`DataSchema`, `ChoiceSchema`, `RosettaEnumerationSchema`, `RosettaFunctionSchema`, `RosettaTypeAliasSchema`) directly. Replace any `toFormValues(node): DataTypeFormValues` with a thin pass-through (the graph node is already AST-shaped). Run `grep -r "form-schemas" packages/visual-editor/src/` and confirm zero matches. Run the full test + type-check + lint gate. This task is gated on Phases 3–8 having completed the editor migration; it cannot land before US1–US6 ship.
