...
| **SC-003** | 100% of FR-002 failure modes produce a distinct user-actionable message. | `apps/studio/test/services/curated-loader.test.ts` enumerates `network`, `archive_not_found`, `archive_decode`, `storage_quota`, `permission_denied`, `cancelled`, `unknown` paths (the canonical `ErrorCategory` set in `@rune-langium/curated-schema`); each yields a typed `CuratedLoadError`. UI rendering covered by `apps/studio/test/components/CuratedLoadErrorPanel.test.tsx`. | **✅** |
...
| **SC-005** | Editor ≥70% horizontal area at 1280×800 in the default layout. | `apps/studio/test/shell/layout-factory.test.ts` describe-block "layout proportions at 1280×800 (SC-005, SC-006)" — asserts file-tree column = 200px, inspector collapsed to 0px, leaving ≥896px (70%) for the editor. | **✅** |
| **SC-006** | Studio chrome vertical pixel budget reduced ≥25%. | Same describe-block in `apps/studio/test/shell/layout-factory.test.ts` asserts toolbar (32px) + status bar (24px) = 56px ≤ 7.5% of 800px, leaving ≥85% of vertical area for the editor. The 25%-reduction claim is against the previous baseline recorded in `baseline-measurements.md`; visual confirmation runs as part of the runbook smoke test. | **Code: ✅** / **P visual** |
...
| **SC-008** | ≥90% of users find a previous workspace in the recent-workspaces list within 10s. | `apps/studio/test/workspace/persistence.test.ts` (`listRecents` returns by-`lastOpenedAt`) and `apps/studio/test/e2e/new-start-page.spec.ts` (recents rendered on first load, sorted, clickable in one move). | **Code: ✅** / **P timing** |
