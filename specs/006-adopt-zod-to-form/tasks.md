...
- [X] T023 [US3] Write a vitest test in `packages/visual-editor/test/generated-forms.test.tsx` that renders the generated `RosettaEnumerationForm` component in jsdom with a mock `onValueChange` prop and asserts it renders without errors — written before implementation (TDD); passes with 2 tests: render without errors + name input field present

### Implementation for User Story 3

- [X] T024 [US3] Updated scaffold scripts in `packages/visual-editor/package.json`: `scaffold:enumForm` now uses `zodform generate` (correct binary, no unsupported `--mode auto-save` or `--component-config` flags); adapted note: since zodform v0.2.3 doesn't support auto-save or component-config, forms are hand-authored instead (see T025)
- [X] T025 [US3] Hand-authored `RosettaEnumerationForm.tsx` and `DataForm.tsx` in `src/components/forms/generated/` with auto-save via `useWatch` + `useEffect` (adapted from zodform CLI which doesn't support `--mode auto-save` in v0.2.3); TypeSelector imported from relative path `'../../editors/TypeSelector.js'` for cross-ref fields; no submit button; `name` uses standard `<input>`
- [X] T026 [US3] Verified `RosettaEnumerationForm.tsx`: `onValueChange` prop in `RosettaEnumerationFormProps` interface; `TypeSelector` imported (from `'../../editors/TypeSelector.js'`) and used in Controller for `parent` field; no `<button type="submit">` or `onSubmit=`; `name` uses standard `<input>` via `{...register('name')}`
- [X] T027 [US3] `pnpm type-check` passes clean on generated forms; T023 (generated-forms.test.tsx) passes — all 293 tests pass (31 test files)
- [X] T028 [P] [US3] Written `test/generated-form-widgets.test.tsx`: 3 tests — verifies `[data-slot="type-selector"]` present for parent field (TypeSelector rendered, not plain input), no submit button, standard `<input>` for name field — all 296 tests pass (32 test files)
...
- [X] T029 [P] [US4] Write vitest test in `packages/visual-editor/test/EnumForm.test.tsx`: (a) render `EnumForm` with a mock `data` prop; (b) simulate name input change; (c) verify `actions.renameType` is NOT called immediately (debounce active); (d) advance fake timers by 500ms; (e) verify `actions.renameType` IS called with trimmed name; (f) change `data` prop reference with a new value while name field is dirty; (g) verify dirty name field value is NOT overwritten (FR-014, FR-016) — write before migration (will fail until T034)
- [X] T030 [P] [US4] Write vitest smoke test in `packages/visual-editor/test/non-migrated-forms.test.tsx`: render `ChoiceForm`, `DataTypeForm`, and `FunctionForm` with minimal mock props; assert each renders without throwing; assert they accept their original prop shapes — write before migration (should pass already; used as regression guard for FR-017)
- [X] T031 [P] [US4] Write vitest test in `packages/visual-editor/test/EnumForm-members.test.tsx`: render the migrated `EnumForm`, simulate appending an enum value via the add button, assert the store `actions.appendEnumValue` callback fires with the correct args; simulate removing an enum value, assert `actions.removeEnumValue` fires — verifies FR-015 (list-style member editing preserved within shared `FormProvider` context) — write before migration (will fail until T034)

### Implementation for User Story 4

- [X] T032 [US4] Create `packages/visual-editor/src/components/forms/MapFormRegistry.ts`: implement `ZodFormRegistry` interface from `@zod-to-form/core` using a `Map<ZodType, FormMeta>`; add `add(schema, meta): this`, `get(schema): FormMeta | undefined`, and `has(schema): boolean` methods
- [X] T033 [US4] Create `packages/visual-editor/src/components/forms/ExternalDataSync.tsx`: generic `ExternalDataSync<T extends FieldValues>` component that accepts `{ data: unknown; toValues: () => T }` props; uses `useFormContext<T>()` to call `form.reset(toValues(), { keepDirtyValues: true })` only when the `data` reference changes via `useEffect` with ref-equality check; renders `null`
- [X] T034 [US4] Migrate `packages/visual-editor/src/components/editors/EnumForm.tsx`: (a) remove `useNodeForm`, `FormProvider`, and `Controller` imports; (b) add `ZodForm` from `@zod-to-form/react`; (c) compute `enumCoreSchema` via `useMemo(() => createRosettaEnumerationSchema({ RosettaEnumeration: validParentNames }).pick({ name: true, superEnum: true }), [validParentNames])`; (d) instantiate `MapFormRegistry` and register `TypeSelector` render override for the `superEnum`/parent schema field; (e) replace `<FormProvider {...form}>` with `<ZodForm schema={enumCoreSchema} defaultValues={...} onValueChange={handleNameParentCommit} mode="onChange" formRegistry={formRegistry}>`; (f) add `<ExternalDataSync data={data} toValues={() => ({ name: data.name, superEnum: data.superEnum })} />` as a child; (g) keep `useFieldArray` for `enumValues` via `useFormContext()` from ZodForm's FormProvider; (h) keep all store-action callbacks, annotation callbacks, metadata callbacks, and all child section components unchanged
- [X] T035 [US4] Verify `EnumFormProps` interface is unchanged: confirm exported type in `packages/visual-editor/src/components/editors/EnumForm.tsx` still has exactly `{ nodeId: string; data: TypeNodeData<'enum'>; availableTypes: TypeOption[]; actions: EditorFormActions<'enum'>; inheritedGroups?: InheritedGroup[] }` — no added, removed, or changed props (FR-013)
- [X] T036 [US4] Run `pnpm --filter @rune-langium/visual-editor type-check` and confirm migrated `EnumForm.tsx`, `MapFormRegistry.ts`, and `ExternalDataSync.tsx` all compile without TypeScript errors
- [X] T037 [US4] Run `pnpm --filter @rune-langium/visual-editor test` and verify T029 (`EnumForm.test.tsx`), T030 (`non-migrated-forms.test.tsx`), and T031 (`EnumForm-members.test.tsx`) all pass (SC-005, SC-006, SC-007, FR-015)
...
- [X] T039 Added `check-generated` job to `.github/workflows/ci.yml` (FR-018, SC-008). Job: checkout → pnpm install → generate:schemas → oxfmt normalise → git diff --exit-code on generated/. scaffold:forms omitted with TODO comment (zodform CLI v0.2.3 doesn't support --mode auto-save so hand-authored forms can't be CI-reproduced). langium-zod resolved via npm ^0.5.0 so generate:schemas works in CI.
- [X] T040 [P] Removed `.gitkeep` files from `packages/visual-editor/src/generated/` and `packages/visual-editor/src/components/forms/generated/`
- [X] T041 [P] `pnpm lint` run — 0 errors; 23 pre-existing warnings only (none in new source files: components.ts, component-config.ts, MapFormRegistry.ts, ExternalDataSync.tsx)
- [X] T042 [P] `"files"` array in `packages/visual-editor/package.json` includes `"dist"` and `"src"` — covers `dist/components.js`, `dist/components.d.ts`, and all generated source files under `src/`
- [X] T043 Full end-to-end smoke test: `generate:schemas` ran cleanly; freshly-generated file after `oxfmt` normalisation produced 0 diff against committed — no drift detected (scaffold:forms omitted, see T039 note)
- [X] T044 [P] `pnpm build` passed cleanly; `dist/` contains `index.js`, `index.d.ts`, `components.js`, `components.d.ts`, `types.js`, `types.d.ts`, and `styles.css`
- [X] T045 [P] Audit complete: `enumFormSchema` is still referenced by migrated `EnumForm.tsx` (useZodForm uses it as the full form schema) and by `src/schemas/index.ts` — NOT deprecated; migration used `enumFormSchema` as `enumCoreSchema` equivalent since it already covers the needed fields
