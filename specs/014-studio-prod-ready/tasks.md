...
- [ ] T032c [DEFERRED] Add `cloneRepository(fs, workspaceId, { url, ref, token })` to `apps/studio/src/services/git-backing.ts` — wraps `git.clone` from isomorphic-git over OPFS, mirrors the existing `initRepo`/`pushBranch` shape. Required before T032 can land.
- [ ] T032d [DEFERRED] Add `WorkspaceManager.createGitBacked({ name, repoUrl, branch, token })` — initialises a workspace folder under OPFS, calls `cloneRepository`, persists a `git-backed` `WorkspaceRecord`. Required before T032 can land.
- [ ] T032e [DEFERRED] Extend `GitHubConnectDialog` (or wrap it) to collect a repo URL + branch *after* successful auth, then call `createGitBacked`. T031's modal is the entry point; this is the second-step UI.
- [ ] T033 [DEFERRED with T032] Component test for the happy path — requires T032c–T032e to exist. The current `GitHubConnect-error-paths.test.tsx` covers the error categories; happy-path test stays valid as a regression target once those land.
- [ ] T034 [DEFERRED with T032] Playwright e2e — same dependency as T033.
