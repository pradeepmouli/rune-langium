...
shape after the Pages Function LSP proxy (`apps/studio/functions/api/lsp/*`)
was deleted in favor of probing `apps/lsp-worker`'s own zone route
(`www.daikonic.dev/rune-studio/api/lsp/*`) directly under `$BASE`.
`/api/parse` (still a real Pages Function) is unaffected and stays under
`$ROOT_BASE`.
...
| 7a | `GET <BASE>/api/lsp/health` | 200 with `langium_loaded:true`, OR 404 warn while apps/lsp-worker's Workers Route is not yet live | `langium_loaded:false` = Worker live but langium import failed (bundle regression) |
| 7b | `POST <BASE>/api/lsp/session` (no `Origin`) | 400/401/403, OR 404/405 warn while apps/lsp-worker's Workers Route is not yet live | 200 = origin allowlist is not enforced (check `ALLOWED_ORIGIN` in `apps/lsp-worker/wrangler.toml`) |
| 7c | `POST <ROOT_BASE>/api/parse` (`{}`) | 400, OR 404/405 warn while the Pages Function is not yet deployed | Any other status means parse validation is broken or unrouted |
